    ...
    <>
      ...
      <div>
          ...
          <div className="w-full h-[20%] bg-white flex justify-center">
            <div className="container flex flex-col md:flex-row justify-between w-full md:w-[90%] p-5 gap-10 items-center">
              ...
              <div className="md:w-1/2 flex flex-col justify-center text-center">
                <h5 className="text-red-500 text-2xl md:text-3xl font-extrabold">PRATIBHA GLOBAL SCHOOL BAGLI</h5>
        ...
        <div className="lg:flex justify-center max-w-full content-center hidden md:flex  w-full ">
          <div
            className={`bg-red-500 h-12 rounded-b-lg flex justify-around w-[100%]  text-center ${isMenuOpen ? 'flex-col' : ''
          }`}
          >
            <Link className="text-base font-semibold px-6 py-4 leading-none hover:bg-white" href="/">
              HOME
            </Link >
            ...
            <Link className='text-base font-semibold px-6 py-4 leading-none hover:bg-white flex" href="/contact-us' href="/contact-us">
            CONTACT
             </Link>
          </div>
</div>
                          ...
                          <ul className="">
                            {values.map((value, index) => (
                              <li key={index} className="py-2">
                                ...
                                <div onClick={() => handleSidebarItemClick(value.path)} className='p-3 shadow-md rounded-lg bg-red-100 text-blue-600'>
                                  {value.displayName}
                                </div>
                              </li>
                            ))}
...
function DropdownNavItem({ name, values }) {
  return (
    <div className="relative group">
      <div className="text-base font-semibold px-6 py-4 leading-none hover:bg-white flex cursor-pointer">
        {name}
        <svg
      ...
      <div className="hidden text-start group-hover:block absolute z-10 bg-gray-100 p-5 rounded-lg w-60">
        <ul className="flex flex-col ">
          {values.map((value, index) => (
             <Link href={value.path}>
            <li key={index} className='p-5 hover:bg-white rounded-lg font-bold'>

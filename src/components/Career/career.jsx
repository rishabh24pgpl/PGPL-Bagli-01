    ...
    return (
        <div className="container mx-auto px-4">
            ...
            <h1 className='font-extrabold text-5xl text-black text-center py-8'>Careers</h1>
                ...
                <p className='font-bold text-4xl p-5 text-black text-center'><span className='text-transparent bg-clip-text bg-gradient-to-r to-red-600 from-red-400'>Work Together&nbsp;</span>to Make Change ..!</p>
            </div>
                        ...
                        <div className="flex flex-col justify-between border-solid border-2 border-orange-100 p-10 shadow-2xl ">
                            <div className="flex justify-between items-center sm:flex justify-between items-center flex-wrap ">
                                ...
                                <div className='flex gap-5 '>
                                    <span className="bg-red-500 rounded-full uppercase text-white text-sm px-4 py-1 font-bold shadow-xl max-w-full"> Remote </span>
                                    <span className="bg-red-500 rounded-full uppercase text-white text-sm px-4 py-1 font-bold shadow-xl ">Full Time </span>
                                </div>
                            </div>
                            <div className="text-sm text-gray-500 flex space-x-1 items-center">
                                ...
                                <span className='flex my-5 flex-wrap'>
                                    {career.skills.map((skill, skillIndex) => (
                                        <div key={skillIndex} className="bg-red-400 uppercase text-white text-sm px-4 py-1 font-bold shadow-xl">
                                            {skill}
                                        </div>
                                    ))}
                                </span>
            ...
            <div className="flex justify-center mt-4">
                {[...Array(Math.ceil(filteredCareers.length / careersPerPage)).keys()].map(number => (
                    <button
                        ...
                        className={`mx-2 px-3 py-1 border rounded-md ${currentPage === number + 1 ? 'bg-blue-500 text-white' : 'bg-white text-blue-500'}`}
                    >
                        {number + 1}
                    </button>
                ))}

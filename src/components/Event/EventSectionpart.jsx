    ...
    <>
      <section className=''>
        <div className="max-w-screen-xl mx-auto px-4 md:px-8 lg:px-12 py-8 md:py-20 bg-white rounded-xl shadow flex flex-col md:flex-row justify-evenly md:justify-evenly items-center md:items-center gap-6 md:gap-12 relative ">
          ...
          <div className="absolute top-0 left-0">
            <Image alt="image" src={'/Abstract Design.svg'} width={100} height={200} />
          </div>

          <div className="items-center md:items-start text-center md:text-left">
            <div className='w-64'>
              <div className="text-zinc-800 px-4 py-2 bg-white rounded-lg border-2 border-neutral-800 text-lg max-w-7 font-medium">Enriching Student Life</div>
            </div>

            <div className="text-4xl lg:text-5xl font-bold mt-4 md:mt-6 text-transparent bg-clip-text bg-gradient-to-r from-red-300 to-red-500 bg-clip-text">Embracing Learning with Discovery and Joy</div>
          </div>
      ...
      <div className="flex flex-wrap justify-center items-center gap-0 sm:gap-10">
        {cardDetails.map((card,i) => (
          <div data-aos={i%2!==0?"fade-right":"fade-up"} onClick={()=>handleRoute(card.title)} className="shadow-[rgba(0,_0,_0,_0.4)_0px_30px_90px] flex flex-col justify-center items-center max-w-sm mx-auto my-8 transition-transform transform-gpu hover:scale-105">
          ...
          <div className="w-[300px] md:w-[370px] bg-transparent -mt-10 shadow-lg rounded-lg overflow-hidden">
              ...
              <div className="flex items-center justify-between py-2 px-3" style={{ backgroundColor: 'rgb(211, 252, 210)' }}>
                  <h1 className="text-red-700  ">{card.title}</h1>
                  <button className="bg-gray-900 text-xs text-white px-2 py-1 font-semibold rounded-md uppercase hover:bg-gray-700" onClick={()=>handleRoute(card.title)}>View more</button>
              </div>
          </div>
      </div>
        ))}

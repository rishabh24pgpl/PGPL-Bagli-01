...
const EventMain = () => {
  ...
  const ourStudent = [
    ...
    },
    {
      ...
      description: 'We foster a strong partnership with parents to ensure seamless communication and mutual support in a child development.',
    }
  ];
  ...
  const activityCard = [
    {
      ...
      description: 'Our program emphasizes Science, Technology, Engineering, and Mathematics (STEM) education, fostering critical thinking and problem-solving skills.',
      bgColor: 'bg-blue-400',
      textColor: 'text-white',
    },
    {
      title: 'Creative Arts Workshop',
      description: 'Our workshop encourages exploration in various creative arts such as painting, sculpting, music, and drama, fostering self-expression and imagination.',
  ...
  return (
    ...
    <>
      <div className='w-full bg-white py-10 bg-cover ' style={{ backgroundImage: "url('/Rect Light.svg')" }}>
        <div className='md:max-w-[1480px] m-auto grid md:grid-cols-2 max-w-[600px]  px-4 md:px-0 mx-auto'>
          ...
          <div className='flex flex-col justify-start gap-4 text-center'>
            <p className='py-2 text-2xl text-[#c80d0d] font-medium'>
              Enriching Student Life</p>
            <h1 className='md:leading-[72px] py-2 md:text-6xl text-4xl font-semibold'>Learning  <span className='text-[#c80d0d]'>with</span> <span className='text-[#c80d0d]'>Discovery  and Joy </span>

            </h1>
            <p className='py-2 text-lg text-center text-gray-600'>Welcome to our Student Life page, where the magic of childhood comes alive through the enriching experiences and adventures that our students embark on each day. At our kindergarten school, we believe that learning goes beyond textbooks, and we strive to create a holistic and engaging environment that nurtures every aspect of a child's development.
            </p>


          </div>
          <div className='relative pr-2'>
            <img src={images[currentSlide]} className='md:order-last order-first rounded-full max-h-full' />
          </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 mx-auto my-5">
          {ourStudent.map((card, index) => (
            ...
            <div key={card.id} className='mx-auto shadow-[5px_5px_0px_0px_rgba(76,175,80)] rounded-lg '>
              <Card
                // imageUrl={card.image}
                title={card.title}
